...
#define MAXPASS 10
  ...
  if (!numberInfeasible) {
	...
	if (columnUpper_[iColumn]-columnLower_[iColumn]<tolerance+1.0e8) {
	  // relax enough so will have correct dj
#if 1
	  columnLower_[iColumn]=max(saveLower[iColumn],
				    columnLower_[iColumn]-100.0*tolerance);
	  columnUpper_[iColumn]=min(saveUpper[iColumn],
				    columnUpper_[iColumn]+100.0*tolerance);
#else
	  if (fabs(columnUpper_[iColumn])<fabs(columnLower_[iColumn])) {
	    if (columnUpper_[iColumn]- 100.0*tolerance>saveLower[iColumn]) {
	      columnLower_[iColumn]=columnUpper_[iColumn]-100.0*tolerance;
	    } else {
	      columnLower_[iColumn]=saveLower[iColumn];
	      columnUpper_[iColumn]=min(saveUpper[iColumn],
					saveLower[iColumn]+100.0*tolerance);
	    }
	  } else {
	    if (columnLower_[iColumn]+100.0*tolerance<saveUpper[iColumn]) {
	      columnUpper_[iColumn]=columnLower_[iColumn]+100.0*tolerance;
	    } else {
	      columnUpper_[iColumn]=saveUpper[iColumn];
	      columnLower_[iColumn]=max(saveLower[iColumn],
					saveUpper[iColumn]-100.0*tolerance);
	    }
	  }
#endif
	} else {
