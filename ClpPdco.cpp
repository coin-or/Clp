...
*/
  ...
  //---------------------------------------------------------------------
  CoinDenseVector rL(n, 0.0);
  CoinDenseVector cL(n, 0.0);
  CoinDenseVector z1(n, 0.0);
  CoinDenseVector dx1(n, 0.0);
  CoinDenseVector dz1(n, 0.0);
  CoinDenseVector r2(n, 0.0);

  // Assign upper bd regions (dummy if no UBs)

  CoinDenseVector rU(nU, 0.0);
  CoinDenseVector cU(nU, 0.0);
  CoinDenseVector x2(nU, 0.0);
  CoinDenseVector z2(nU, 0.0);
  CoinDenseVector dx2(nU, 0.0);    
  CoinDenseVector dz2(nU, 0.0);

  //---------------------------------------------------------------------
  // Initialize x, y, z, objective, etc.
  //---------------------------------------------------------------------
  CoinDenseVector dx(n, 0.0);
  ...
  for (int k=0; k<nlow; k++){
    ...
    x1_elts[low[k]] = max( x_elts[low[k]] - bl[low[k]], x0min  );
    z1_elts[low[k]] = max( z_elts[low[k]], z0min  );
  }
  for (int k=0; k<nupp; k++){
    x_elts[upp[k]]  = min( x_elts[upp[k]], bu[upp[k]]);
    x2_elts[upp[k]] = max(bu[upp[k]] -  x_elts[upp[k]], x0min  );
    z2_elts[upp[k]] = max(-z_elts[upp[k]], z0min  );
  }
  ...
  */
  ...
  while(PDitns < maxitn) {
	...
	*/
    ...
    sprintf(str2, "%6.3f%6.3f" , stepx       , stepz       );
