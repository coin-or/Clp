    ...
    */
    while (problemStatus_ < 0) {
      ...
      if (problemStatus_ >= 0) {
        ...
        int status = eventHandler_->event(ClpEventHandler::endInPrimal);
      ...
      if (abcNonLinearCost_->numberInfeasibilities() > 0 && !abcProgress_.initialWeight_ && !ifValuesPass && infeasibilityCost_ == 1.0e10
        ...
        if (n) {
          ...
          // was infeasibilityCost_ = std::min(std::max(1000.0 * take, 1.0e8), 1.0000001e10);
          infeasibilityCost_ = std::min(std::max(1000.0 * take, 1.0e3), 1.0000001e10);
#ifdef CLP_USEFUL_PRINTOUT
...
/*
  ...
  +3 max iterations
*/
int AbcSimplexPrimal::whileIterating(int valuesOption)
{
#if 1
      ...
      if (handler_->logLevel() == 63) {
        ...
        for (int i = 0; i < numberTotal_; i++) {
          if (getInternalStatus(i) == isFixed)
            continue;
          largestCost = std::max(largestCost, fabs(abcCost_[i]));
          double dj = abcDj_[i];
          if (getInternalStatus(i) == atLowerBound)
            dj = -dj;
          largestDj = std::max(largestDj, fabs(dj));
          if (largestGoodDj < dj) {
            largestGoodDj = dj;
            iLargest = i;
          }
...
void AbcSimplexPrimal::statusOfProblemInPrimal(int type)
{
...
#define CLP_CAUTION 1
#endif
#if CLP_CAUTION
  double lastAverageInfeasibility = sumDualInfeasibilities_ / static_cast< double >(numberDualInfeasibilities_ + 1);
#endif
  ...
  if (problemStatus_ > -3 || problemStatus_ == -4) {
      ...
      if (factorStatus) {
        if (type != 1 || largestPrimalError_ > 1.0e3
          ...
          if (numberPivots <= 1) {
            ...
            double newTolerance = std::max(0.5 + 0.499 * randomNumberGenerator_.randomDouble(), abcFactorization_->pivotTolerance());
            abcFactorization_->pivotTolerance(newTolerance);
          } else {
            // Go to safe
    ...
    if (problemStatus_ != -4)
      problemStatus_ = -3;
    if (abcProgress_.realInfeasibility_[0] < 1.0e-1 && primalTolerance_ == 1.0e-7 && abcProgress_.iterationNumber_[0] > 0 && abcProgress_.iterationNumber_[CLP_PROGRESS - 1] - abcProgress_.iterationNumber_[0] > 25) {
    ...
    if (ifValuesPass && numberIterations_ == baseIteration_) {
      ...
      while (numberOut) {
        ...
        if ((lastSumInfeasibility < incomingInfeasibility_ || badInfeasibility > (std::max(10.0, 100.0 * lastSumInfeasibility)))
          ...
          numberOut = std::min(1000, numberOut);
    ...
    double test2 = 1.0e-1;
#endif
    if (!lastSumInfeasibility && sumInfeasibility > 1.0e3 * primalTolerance_ && lastAverageInfeasibility < test2 && numberPivots > 10 && !ifValuesPass)
      reason2 = 3;
    if (lastSumInfeasibility < 1.0e-6 && sumInfeasibility > 1.0e-3 && numberPivots > 10 && !ifValuesPass)
      reason2 = 4;
#endif
    ...
    if (reason2) {
      ...
      if (numberPivots) {
        ...
        if (reason2 < 3) {
          // Go to safe
          abcFactorization_->pivotTolerance(std::min(0.99, 1.01 * abcFactorization_->pivotTolerance()));
          forceFactorization_ = 1; // a bit drastic but ..
        } else if (forceFactorization_ < 0) {
          forceFactorization_ = std::min(numberPivots / 4, 100);
        } else {
          forceFactorization_ = std::min(forceFactorization_,
            std::max(3, numberPivots / 4));
        }
        pivotRow_ = -1; // say no weights update
        changeMade_++; // say change made
        if (numberPivots == 1) {
  ...
  if ((moreSpecialOptions_ & 2048) != 0 && !numberPrimalInfeasibilities_ && numberDualInfeasibilities_) {
    double average = sumDualInfeasibilities_ / (static_cast< double >(numberDualInfeasibilities_));
    if (abcProgress_.lastIterationNumber(2) == numberIterations_ && ((abcProgress_.timesFlagged() > 2 && average < 1.0e-1) || abcProgress_.timesFlagged() > 20)) {
      numberDualInfeasibilities_ = 0;
      sumDualInfeasibilities_ = 0.0;
      problemStatus_ = 3;
      loop = 0;
  ...
  if (loop >= 0) {
    ...
    if (abcNonLinearCost_->numberInfeasibilities() && abcProgress_.badTimes() > 5 && abcProgress_.oddState() < 10 && abcProgress_.oddState() >= 0) {
  ...
  if (!lastNumberInfeasibility && sumInfeasibility && numberPivots > 1 && !ifValuesPass && !takenAction && abcProgress_.lastObjective(0) >= abcProgress_.lastObjective(CLP_PROGRESS - 1)) {
  ...
  if (lastAverageInfeasibility < 2.0 * dualTolerance_) {
    double averageInfeasibility = sumDualInfeasibilities_ / static_cast< double >(numberDualInfeasibilities_ + 1);
  ...
  if ((dualFeasible() || problemStatus_ == -4) && !ifValuesPass) {
    // see if extra helps
    if (abcNonLinearCost_->numberInfeasibilities() && (abcNonLinearCost_->sumInfeasibilities() > 1.0e-3 || sumOfRelaxedPrimalInfeasibilities_)
      && !alwaysOptimal) {
      ...
      if (abcNonLinearCost_->numberInfeasibilities() == 0) {
        ...
        if ((infeasibilityCost_ >= 1.0e18 || numberDualInfeasibilities_ == 0) && perturbation_ == 101
          ...
          goToDual = true;
          abcFactorization_->pivotTolerance(std::max(0.5, abcFactorization_->pivotTolerance()));
        }
        if (!goToDual) {
	  ...
	  for (int i=0;i<numberTotal_;i++) 
	    largestDifference=std::max(largestDifference,fabs(abcCost_[i]-costSaved_[i]));
	  if (handler_->logLevel()==63)
    ...
    if (problemStatus_ == -5) {
      ...
      if (type == 3 && !ifValuesPass) {
        ...
        unflag();
        abcProgress_.clearTimesFlagged();
        if (sumDualInfeasibilities_ < 1.0e-3 || (sumDualInfeasibilities_ / static_cast< double >(numberDualInfeasibilities_)) < 1.0e-5 || abcProgress_.lastIterationNumber(0) == numberIterations_) {
          if (!numberPrimalInfeasibilities_) {
            if (numberTimesOptimal_ < 4) {
              numberTimesOptimal_++;
              changeMade_++; // say change made
  ...
  if ((specialOptions_ & 8192) != 0)
    goToDual = false;
#endif
  if (goToDual || (numberIterations_ > 1000 + baseIteration_ && largestPrimalError_ > 1.0e6 && largestDualError_ > 1.0e6)) {
  ...
  if (valuesChunk > 0) {
    ...
    if (numberIterations_ == baseIteration_ && numberDualInfeasibilitiesWithoutFree_ + 1000 < numberDualInfeasibilities_) {
      doFixing = true;
...
*/
void AbcSimplexPrimal::primalRow(CoinIndexedVector *rowArray,
  ...
  pivotStruct &stuff)
{
  ...
  */
  ...
  double maximumMovement;
  if (way > 0.0)
    maximumMovement = std::min(1.0e30, upperIn - valueIn);
  else
  ...
  if (pivotRow >= 0) {
    ...
    if (distance - minimumTheta * fabs(alpha) < -primalTolerance_)
      minimumTheta = std::max(0.0, (distance + 0.5 * primalTolerance_) / fabs(alpha));
    // will we need to increase tolerance
    //double largestInfeasibility = primalTolerance_;
...
*/
void AbcSimplexPrimal::primalColumn(CoinPartitionedVector *updates,
...
*/
int AbcSimplexPrimal::updatePrimalsInPrimal(CoinIndexedVector *rowArray,
  double theta,
...
// Perturbs problem
void AbcSimplexPrimal::perturb(int /*type*/)
{
  if (perturbation_ > 100)
  ...
  if ((!numberIterations_ || initialSumInfeasibilities_ == 1.23456789e-5) && perturbation_ == 50) {
    ...
    int numberTotal = std::max(numberRows_, numberColumns_);
  ...
  if (perturbation_ >= 50) {
    ...
    for (i = 0; i < numberColumns_ + numberRows_; i++) {
      if (abcUpper_[i] > abcLower_[i] + primalTolerance_) {
        ...
        value = std::min(value, abcUpper_[i] - abcLower_[i]);
#if 1
        if (value) {
          perturbation += value;
          numberNonZero++;
  ...
  for (iSequence = 0; iSequence < numberRows_ + numberColumns_; iSequence++) {
    if (getInternalStatus(iSequence) == basic) {
      ...
      if (upperValue > lowerValue + tolerance) {
        ...
        value = std::max(value, primalTolerance_);
...
// un perturb
bool AbcSimplexPrimal::unPerturb()
{
  if (perturbation_ != 101)
    return false;
...
void AbcSimplexPrimal::alwaysOptimal(bool onOff)
{
  if (onOff)
    specialOptions_ |= 1;
  else
...
bool AbcSimplexPrimal::alwaysOptimal() const
{
  return (specialOptions_ & 1) != 0;
}
// Flatten outgoing variables i.e. - always to exact bound
void AbcSimplexPrimal::exactOutgoing(bool onOff)
{
  if (onOff)
    specialOptions_ |= 4;
  else
...
bool AbcSimplexPrimal::exactOutgoing() const
{
...
*/
int AbcSimplexPrimal::pivotResult(int ifValuesPass)
{
...
#endif
    if (!ifValuesPass && (saveDj * dualIn_ < test1 || fabs(saveDj - dualIn_) > checkValue * (1.0 + fabs(saveDj)) || fabs(dualIn_) < test2)) {
      if (!(saveDj * dualIn_ > 0.0 && std::min(fabs(saveDj), fabs(dualIn_)) > 1.0e5)) {
        ...
        if (lastGoodIteration_ != numberIterations_) {
          ...
          if (saveDj * dualIn_ < test1 || fabs(saveDj - dualIn_) > 2.0e-1 * (1.0 + fabs(dualIn_)) || fabs(dualIn_) < test2) {
  ...
  // Make safer?
    ...
    if (whatNext == 1) {
      ...
      returnCode = 3;
    } else if (numberIterations_ == lastGoodIteration_ + 2 * abcFactorization_->maximumPivots()) {
...
/*
  ...
  +3 max iterations (iteration done)
*/
int AbcSimplexPrimal::pivotResult4(int ifValuesPass)
{
...
int AbcSimplexPrimal::doFTUpdate(CoinIndexedVector *vector[4])
{
...
/* The primals are updated by the given array.
   costs are changed
*/
void AbcSimplexPrimal::updatePrimalsInPrimal(CoinIndexedVector &rowArray,
 ...
 */
void AbcSimplexPrimal::createUpdateDuals(CoinIndexedVector &rowArray,
...
void AbcSimplexPrimal::updatePartialUpdate(CoinIndexedVector &partialUpdate)
{
#ifndef ABC_USE_COIN_FACTORIZATION
  CoinAbcFactorization *factorization = dynamic_cast< CoinAbcFactorization * >(abcFactorization_->factorization());
  if (factorization)
...
// Create primal ray
void AbcSimplexPrimal::primalRay(CoinIndexedVector *rowArray)
{
  delete[] ray_;
...
/* Get next superbasic -1 if none,
   ...
   if 2 uses list.
*/
int AbcSimplexPrimal::nextSuperBasic(int superBasicType,
  ...
  while (!finished) {
    ...
    if (superBasicType > 1) {
          ...
          if (!flagged(iColumn)) {
            if (getInternalStatus(iColumn) == superBasic) {
              if (fabs(abcSolution_[iColumn] - abcLower_[iColumn]) <= primalTolerance_) {
                ...
                work[number] = -std::min(0.1 * (abcSolution_[iColumn] - abcLower_[iColumn]),
      ...
      for (; iColumn < numberRows_ + numberColumns_; iColumn++) {
        if (!flagged(iColumn)) {
          if (getInternalStatus(iColumn) == superBasic || getInternalStatus(iColumn) == isFree) {
            if (fabs(abcSolution_[iColumn] - abcLower_[iColumn]) <= primalTolerance_) {
              abcSolution_[iColumn] = abcLower_[iColumn];
              setInternalStatus(iColumn, atLowerBound);
            } else if (fabs(abcSolution_[iColumn] - abcUpper_[iColumn])
...
void AbcSimplexPrimal::clearAll()
{
