        ...
        if (numberIterations_ > saveNumberIterations || problemStatus_ >= 0) {
          ...
          if (pivotTolerance < abcFactorization_->pivotTolerance()) {
            ...
            if (!abcFactorization_->pivots())
              abcFactorization_->minimumPivotTolerance(std::min(0.25, 1.05 * abcFactorization_->minimumPivotTolerance()));
          }
          break;
        } else {
    ...
    if (nextFree >= 0) {
      ...
      for (int i = 0; i < number; i++) {
        ...
        if (alpha > 1.0e-3) {
          ...
          infeasibility = std::min(maxInfeas, infeasibility);
    ...
    for (iSequence = 0; iSequence < numberTotal_; iSequence++) {
      ...
      if (status == atUpperBound || status == atLowerBound) {
        ...
        if (lowerValue > -largeValue_ || upperValue < largeValue_) {
          if (true || lowerValue - value > -0.5 * currentDualBound_ || upperValue - value < 0.5 * currentDualBound_) {
            if (fabs(lowerValue - value) <= fabs(upperValue - value)) {
              if (upperValue > lowerValue + testBound) {
                ...
                if (upperSaved_[iSequence] - abcUpper[iSequence] > abcLower[iSequence] - lowerSaved_[iSequence]) {
                  setFakeBound(iSequence, AbcSimplexDual::upperFake);
                  assert(abcLower[iSequence] == lowerSaved_[iSequence]);
                  assert(abcUpper[iSequence] < upperSaved_[iSequence]);
                } else {
              ...
              if (lowerValue < upperValue - testBound) {
                ...
                if (upperSaved_[iSequence] - abcUpper[iSequence] > abcLower[iSequence] - lowerSaved_[iSequence]) {
                  setFakeBound(iSequence, AbcSimplexDual::upperFake);
                  //assert (abcLower[iSequence]==lowerSaved_[iSequence]);
                  abcLower[iSequence] = std::max(abcLower[iSequence], lowerSaved_[iSequence]);
                  assert(abcUpper[iSequence] < upperSaved_[iSequence]);
                } else {
                  setFakeBound(iSequence, AbcSimplexDual::lowerFake);
                  assert(abcLower[iSequence] > lowerSaved_[iSequence]);
                  //assert (abcUpper[iSequence]==upperSaved_[iSequence]);
                  abcUpper[iSequence] = std::min(abcUpper[iSequence], upperSaved_[iSequence]);
                }
              }
            }
          } else {
...
*/
void AbcSimplexDual::dualColumn1(bool doAll)
{
    ...
    for (int i = 0; i < number; i++) {
      ...
      if ((type & 4) == 0) {
        ...
        if (oldValue > currentDualTolerance_) {
          ...
          if (fabs(piValue) > std::max(10.0 * currentAcceptablePivot_, 1.0e-5)) {
            keep = true;
          } else {
            keep = false;
...
void AbcSimplexDual::dualColumn2Most(dualColumnResult &result)
{
  ...
  tentativeTheta = std::max(10.0 * upperTheta_, 1.0e-7);
  ...
  while (tentativeTheta < 1.0e22) {
    ...
    printf("Iteration %d pass %d dualOut %g thru %g increase %g numberRemain %d\n",
      ...
      numberRemaining);
#endif
    if (totalThru + thruThis > fabs(dualOut_) - fabs(1.0e-9 * dualOut_) - 1.0e-12 || (increaseInObjective + increaseInThis < 0.0 && bestSequence >= 0) || !numberRemaining) {
      ...
      for (iTry = 0; iTry < MAXTRY; iTry++) {
        ...
        if (((increaseInObjective < 0.0 && increase < 0.0) || (theta > 1.0e9 && lastTheta < 1.0e-5))
          && lastSequence >= 0) {
          // back
...
void AbcSimplexDual::dualColumn2()
{
  ...
  if (abcFactorization_->pivots() > 10 || (abcFactorization_->pivots() && sumDualInfeasibilities_))
  ...
  if (abcFactorization_->pivots()) {
    double value = std::max(currentAcceptablePivot_, 1.0e-12 * fabs(dualOut_));
    currentAcceptablePivot_ = std::min(1.0e2 * currentAcceptablePivot_, value);
  }
...
#ifdef MORE_CAREFUL
  // If we have done pivots and things look bad set alpha_ 0.0 to force factorization
  if ((badSumPivots || fabs(theta_ * badFree) > 10.0 * currentDualTolerance_) && abcFactorization_->pivots()) {
#if ABC_NORMAL_DEBUG > 0
    if (handler_->logLevel() > 1)
  ...
  if (sequenceIn_ >= 0) {
    ...
    if (numberIterations_ >= normalDualColumnIteration_) {
      ...
      double smallMove = std::max(fabs(rawObjectiveValue_), 1.0e-3);
...
int AbcSimplex::gutsOfSolution(int type)
{
  ...
  if (numberIterations_ && (forceFactorization_ > 2 || forceFactorization_ < 0 || abcFactorization_->pivotTolerance() < 0.9899999999) && (oldLargestDualError || oldLargestPrimalError)) {
    ...
    if ((largestPrimalError_ > 1.0e3 && oldLargestPrimalError * 1.0e2 < largestPrimalError_) || (useDualError > 1.0e3 && useOldDualError * 1.0e2 < useDualError)) {
      ...
      if (pivotTolerance < 0.1)
        ...
        abcFactorization_->pivotTolerance(std::min(0.99, pivotTolerance * factor));
  ...
  if (progress_.iterationNumber_[0] > 0 && progress_.iterationNumber_[CLP_PROGRESS - 1] - progress_.iterationNumber_[0] < CLP_PROGRESS * 3 && abcFactorization_->pivotTolerance() < 0.25 && notChanged) {
    double pivotTolerance = abcFactorization_->pivotTolerance();
...
int AbcSimplexDual::makeNonFreeVariablesDualFeasible(bool changeCosts)
{
  ...
  for (int iSequence = 0; iSequence < numberTotal_; iSequence++) {
    ...
    if (iStatus < 4) {
      ...
      if (dj < dualT) {
        largestCost = std::max(fabs(abcPerturbation_[iSequence]), largestCost);
      }
    } else if (iStatus != 7) {
      largestCost = std::max(fabs(abcPerturbation_[iSequence]), largestCost);
    }
  ...
  for (int iSequence = 0; iSequence < numberTotal_; iSequence++) {
    ...
    if (iStatus < 4) {
      ...
      if (dj < dualT && changeCosts) {
        ...
        if ((changeInObjective > 1.0e3 || gap > std::max(0.5 * dualBound_, 1.0e3)) && dj > -1.0e-1) {
          // change cost
...
void AbcSimplexDual::statusOfProblemInDual(int type)
{
  ...
  if (!type) {
    ...
    // but use 0.1
    double newTolerance = std::max(0.1, saveData_.pivotTolerance_);
  ...
  if (type != 3 && (numberPivots > dontFactorizePivots_ || numberIterations_ == baseIteration_)) {
    ...
    if ((largestPrimalError_ > 1.0e1 || largestDualError_ > 1.0e1)
      ...
      double newTolerance = std::min(1.1 * abcFactorization_->pivotTolerance(), 0.99);
      abcFactorization_->pivotTolerance(newTolerance);
    }
    ...
    if ((numberEarly_ & 0xffff) <= 5 || !usefulArray_[ABC_NUMBER_USEFUL - 1].getNumElements()) {
      ...
      newEarly = std::min(savedEarly, 1 + 2 * newEarly);
      ...
      if (!bad) {
        ...
        factorizationStatus = abcEarlyFactorization_->replaceColumns(this, vector,
...
#define GOOD_EARLY
  ...
  if ((moreSpecialOptions_ & 16384) != 0 && (moreSpecialOptions_ & 8192) == 0) {
    if (numberIterations_ == baseIteration_) {
      ...
      if (!sumPrimalInfeasibilities_ && sumDualInfeasibilities_ > 1.0 && numberRows_ * 4 > numberColumns_) {
        // do primal
        // mark so can perturb
    ...
    } else if ((numberIterations_ > 1000 && numberIterations_ * 10 > numberRows_ && numberIterations_ * 4 < numberRows_) || (sumPrimalInfeasibilities_ + sumDualInfeasibilities_ > 1.0e10 * (initialSumInfeasibilities_ + 1000.0) && numberIterations_ > 10000 && sumDualInfeasibilities_ > 1.0)) {
      if (sumPrimalInfeasibilities_ + 1.0e2 * sumDualInfeasibilities_ > 2000.0 * (initialSumInfeasibilities_ + 100.0)) {
#if ABC_NORMAL_DEBUG > 0
        ...
        if (sumPrimalInfeasibilities_ * initialNumberInfeasibilities_ > 100.0 * (initialSumInfeasibilities_ + 10.0) * numberPrimalInfeasibilities_) {
#if ABC_NORMAL_DEBUG > 0
    ...
    ignoreStuff = 2;
  {
    ...
    if ((lastObj > thisObj + 1.0e-3 * std::max(fabs(thisObj), fabs(lastObj)) + 1.0e-4 || (sumPrimalInfeasibilities_ > 10.0 * lastSumPrimalInfeasibilities + 1.0e3 && sumDualInfeasibilities_ > 10.0 * std::max(lastSumDualInfeasibilities, 1.0)))
      && !ignoreStuff) {
#if ABC_NORMAL_DEBUG > 0
...
#endif
      numberBlackMarks++;
      if (largestDualError_ > 1.0e-3 || largestPrimalError_ > 1.0e-3 || (sumDualInfeasibilities_ > 10.0 * std::max(lastSumDualInfeasibilities, 1.0) && firstFree_ < 0)) {
#if ABC_NORMAL_DEBUG > 0
  ...
  if (!type && (specialOptions_ & COIN_CBC_USING_CLP) == 0 && firstFree_ < 0) {
    ...
    for (int i = 0; i < numberTotal_; i++) {
      ...
      if (status == atLowerBound && djValue < -currentDualTolerance_) {
        ...
        if (gap > 100000000.0)
          badValue = -djValue * std::min(gap, 1.0e10);
    ...
    if (number && ignoreStuff != 2) {
      ...
      int numberToDo = std::min(number, numberRows_ / 10); // ??
#if PRINT_PAN > 1
      ...
      for (int i = 0; i < numberToDo; i++) {
        ...
        firstFree_ = std::min(firstFree_, iSequence);
      }
#if PRINT_PAN > 1
      if (n)
        printf("\n");
  ...
  if ((largestPrimalError_ > 1.0e15 || largestDualError_ > 1.0e15) && numberIterations_ > 1000) {
    ...
    double newTolerance = std::max(0.995 * abcFactorization_->pivotTolerance(), saveData_.pivotTolerance_);
    if (!type)
...
#endif
#endif
    if (lastObj < thisObj - 1.0e-5 * std::max(fabs(thisObj), fabs(lastObj)) - 1.0e-3 && saveType < 9) {
    ...
    if (sumPrimalInfeasibilities_ && sumPrimalInfeasibilities_ < 1.0e5) {
      ...
      if (backIteration > 0 && numberIterations_ - backIteration < 9 * CLP_PROGRESS) {
        if (abcFactorization_->pivotTolerance() < 0.9) {
          // up tolerance
          abcFactorization_->pivotTolerance(std::min(abcFactorization_->pivotTolerance() * 1.05 + 0.02, 0.91));
  ...
  if (primalFeasible() && ignoreStuff != 2) {
    if (numberChangedBounds <= 0 && !numberDualInfeasibilities_) {
      ...
      if (lastCleaned_ < numberIterations_ && /*numberTimesOptimal_*/ stateDualColumn_ < 4 && (numberChanged_ || (specialOptions_ & 4096) == 0)) {
        ...
        if ((specialOptions_ & 2097152) == 0) {
          ...
          if (numberTimesOptimal_ == 1) {
            ...
            if (numberTimesOptimal_ == 2) {
              ...
              abcFactorization_->zeroTolerance(std::min(abcFactorization_->zeroTolerance(), 1.0e-15));
            }
#if ABC_NORMAL_DEBUG > 0
            printf("changing dual tolerance from %g to %g (numberTimesOptimal_==%d)\n",
  ...
  if ((specialOptions_ & 1024) != 0) {
    ...
    if (largestPrimalError_ * largestDualError_ > 1.0e2) {
      ...
      && rawObjectiveValue_ > std::min(1.0e15, 1.0e3 * limit)) {
      looksBad = 2;
    }
    if (looksBad) {
      if (abcFactorization_->pivotTolerance() < 0.9) {
  ...
  if (numberBlackMarks > 0 && numberIterations_ > baseIteration_) {
    ...
    if (maxFactor > 10) {
      if ((stateOfProblem_ & PESSIMISTIC) == 0 || true) {
        if (largestDualError_ > 10.0 * std::max(lastDualError_, 1.0e-6)
          || largestPrimalError_ > 10.0 * std::max(lastPrimalError_, 1.0e-6))
          maxFactor = std::min(maxFactor, 20);
        forceFactorization_ = std::max(1, (maxFactor >> 1));
...
bool AbcSimplexDual::checkCutoff(bool computeObjective)
{
  ...
  if (fabs(limit) < 1.0e30 && objValue > limit && !numberAtFakeBound()) {
    bool looksInfeasible = !numberDualInfeasibilities_;
    if (objValue > limit + fabs(0.1 * limit) + 1.0e2 * sumDualInfeasibilities_ + 1.0e4 && sumDualInfeasibilities_ < largestDualError_ && numberIterations_ > 0.5 * numberRows_ + 1000)
...
int AbcSimplexDual::flipBounds()
{
  ...
  if (numberFlipped) {
    ...
    const double *COIN_RESTRICT dj = abcDj_;
      ...
      for (int i = 0; i < number; i++) {
        ...
        assert((internalStatus_[iSequence] & 7) == 0 || (internalStatus_[iSequence] & 7) == 1);
        double mult = multiplier[iStatus];
        ...
        if (djValue < -currentDualTolerance_) {
          ...
          if (iStatus == 0) {
            ...
            if (cost[iSequence] < originalCost[iSequence]) {
              double difference = std::max(cost[iSequence] - originalCost[iSequence], djValue);
...
#if CLEANUP_DJS
            if (cost[iSequence] > originalCost[iSequence]) {
              double difference = std::min(cost[iSequence] - originalCost[iSequence], -djValue);
              dj[iSequence] -= difference;
              cost[iSequence] -= difference;
            }
#endif
  ...
  if (objectiveChange_ + fabs(movementOld * dualIn_) < -std::max(1.0e-5, 1.0e-12 * fabs(rawObjectiveValue_))) {
#if ABC_NORMAL_DEBUG > 3
    if (handler_->logLevel() & 32)
    ...
    for (int iSequence = 0; iSequence < numberTotal_; iSequence++) {
      ...
      if (fakeStatus != AbcSimplexDual::noFake) {
        ...
        if (fakeStatus == AbcSimplexDual::upperFake) {
          ...
          } else if (status == AbcSimplex::isFree || status == AbcSimplex::superBasic) {
...
void AbcSimplex::checkDualSolutionPlusFake()
{
    ...
    if (getInternalStatus(iSequence) != basic) {
      ...
      if (distanceUp > primalTolerance_) {
        ...
        if (distanceDown <= primalTolerance_) {
          ...
          if (value < 0.0) {
            ...
            if (abcCost[iSequence] > originalCost[iSequence]) {
              double difference = std::min(abcCost[iSequence] - originalCost[iSequence], value);
        ...
        if (value > 0.0) {
          ...
          // adjust cost etc
          if (abcCost[iSequence] < originalCost[iSequence]) {
            double difference = std::max(abcCost[iSequence] - originalCost[iSequence], value);
  ...
  } else if ((numberSuperBasicWithDj || (abcProgress_.lastIterationNumber(0) <= 0)) && algorithm_ > 0) {
...
#endif
// Perturbs problem
void AbcSimplexDual::perturbB(double /*factorIn*/, int /*type*/)
{
  ...
  if (perturbation_ >= 50) {
    ...
    int whichOne = std::min(perturbation_ - 51, 10);
    if (whichOne < 0) {
      if (numberRows_ < 5000)
  ...
  perturbation = std::max(1.0e-10, maximumFraction);
  double *COIN_RESTRICT cost = abcCost_;
  bool allSame = true;
  ...
  for (int iSequence = maximumAbcNumberRows_; iSequence < numberTotal_; iSequence++) {
    ...
    if (lo < up) {
      double value = fabs(cost[iSequence]);
      perturbation = std::max(perturbation, value);
      if (value) {
        smallestNonZero = std::min(smallestNonZero, value);
      }
  ...
  const double m1 = 0.5;
  double smallestAllowed = overallMultiplier * std::min(1.0e-2 * dualTolerance_, maximumFraction);
  double largestAllowed = overallMultiplier * std::max(1.0e3 * dualTolerance_, maximumFraction * averageCost);
  // smaller if in BAB
  //if (inCbcOrOther)
  ...
  for (int iSequence = maximumAbcNumberRows_; iSequence < numberTotal_; iSequence++) {
    if (lower[iSequence] < upper[iSequence] && getInternalStatus(iSequence) != basic) {
      ...
      if (lower[iSequence] > -largeValue_) {
        if (fabs(lower[iSequence]) < fabs(upper[iSequence])) {
          value *= perturbationValue;
  ...
  if (perturbation_ >= 50) {
    ...
    for (int iColumn = maximumAbcNumberRows_; iColumn < numberTotal_; iColumn++) {
      ...
      if (lo < up) {
        double value = fabs(abcCost_[iColumn]);
        perturbation = std::max(perturbation, value);
        if (value) {
          smallestNonZero = std::min(smallestNonZero, value);
        }
  ...
  for (int iColumn = maximumAbcNumberRows_; iColumn < numberTotal_; iColumn++) {
    if (abcLower_[iColumn] < abcUpper_[iColumn]
      ...
      if (abcLower_[iColumn] > -largeValue_) {
        if (fabs(abcLower_[iColumn]) < fabs(abcUpper_[iColumn])) {
          value *= (1.0 - m1 + m1 * randomNumberGenerator_.randomDouble());
          value2 *= (1.0 - m1 + m1 * randomNumberGenerator_.randomDouble());
        } else {
          //value *= -(1.0-m1+m1*randomNumberGenerator_.randomDouble());
...
int AbcSimplexDual::bounceTolerances(int type)
{
  ...
  if (type == -1) {
    ...
    CoinAbcMemcpy(abcCost_, costSaved_, numberTotal_);
    if (!perturbationFactor_) {
      //currentDualBound_=std::min(dualBound_,1.0e11);
    ...
    if (stateDualColumn_ == -2) {
      double newTolerance = std::min(1.0e-1, dualTolerance_ * 100.0);
#if ABC_NORMAL_DEBUG > 0
      ...
      if (stateDualColumn_ > 1) {
        normalDualColumnIteration_ = numberIterations_ + std::max(10, numberRows_ / 4);
#if ABC_NORMAL_DEBUG > 0
...
int AbcSimplexDual::noPivotRow()
{
  ...
  int useNumberFake;
  int returnCode = 0;
  if (numberPivots <= std::max(dontFactorizePivots_, 20) && (specialOptions_ & 2048) != 0 && currentDualBound_ >= 1.0e8) {
    specialCase = true;
...
#define CLP_CHECK_NUMBER_PIVOTS 10
        ...
        if ((specialOptions_ & (1024 + 16384)) != 0 && !problemStatus_ && abcFactorization_->pivots()) {
          ...
          for (i = 0; i < numberRows_; i++) {
            if (rhs[i] < abcLower_[i] - primalTolerance_ || rhs[i] > abcUpper_[i] + primalTolerance_) {
              bad2 = true;
#ifdef CHECK_ACCURACY
          ...
          for (int i = 0; i < numberTotal_; i++) {
            if (abcSolution_[i] < abcLower_[i] - primalTolerance_ || abcSolution_[i] > abcUpper_[i] + primalTolerance_) {
              bad2 = true;
#ifdef CHECK_ACCURACY
...
void AbcSimplexDual::dualPivotColumn()
{
  ...
  if ((stateOfProblem_ & VALUES_PASS) != 0) {
    ...
    if (sequenceIn_ < 0) {
      ...
      if ((directionOut_ < 0 && lowerOut_ > -1.0e30) || (directionOut_ > 0 && upperOut_ < 1.30)) {
...
int AbcSimplexDual::checkReplace()
{
  ...
  if (!stateOfIteration_) {
    ...
    if (updateStatus == 2 && !abcFactorization_->pivots() && fabs(alpha_) > 1.0e-5)
      updateStatus = 4;
    if (updateStatus == 1 || updateStatus == 4) {
      ...
      if (abcFactorization_->pivots() && ((moreSpecialOptions_ & 16) == 0 || abcFactorization_->pivots() > 4)) {
        problemStatus_ = -2; // factorize now
        returnCode = -2;
      ...
      if (abcFactorization_->pivots() < 0.5 * abcFactorization_->maximumPivots() && abcFactorization_->pivots() < 200)
...
#endif
  if (abcFactorization_->pivots() <= dontFactorizePivots_ && acceptablePivot_ <= 1.0e-8 && lastFirstFree_ < 0) {
    ...
    if (largestGap_)
      dualTest = std::min(largestGap_, dualTest);
    ...
    if (((specialOptions_ & 4096) != 0 || fabs(alpha_) < 1.0e-11) && currentDualBound_ >= dualTest) {
      ...
      if (valueOut_ > upperOut_ + testValue || valueOut_ < lowerOut_ - testValue
        ...
        if (sumPrimalInfeasibilities_ < 1.0e-3 || sumDualInfeasibilities_ > 1.0e-5 || (sumPrimal < 1.0e5 && (specialOptions_ & 1024) != 0 && abcFactorization_->pivots())) {
...
void *abc_parallelManager(void *simplex)
{
  ...
  while (true) {
    ...
    case 5:
      which[2] = -1;
      threadInfo->result = dual->abcMatrix()->dualColumn1Row(which[1], COIN_DBL_MAX, which[2],
        *dual->usefulArray(dual->arrayForBtran()),
        *dual->usefulArray(dual->arrayForTableauRow()),
        *dual->usefulArray(dual->arrayForDualColumn()));
      break;
...
void AbcSimplex::startParallelStuff(int type)
{
  /*
...
int AbcSimplex::stopParallelStuff(int type)
{
...
int AbcSimplexDual::dual()
{
  ...
  if (!lastDualBound_) {
    ...
    currentDualBound_ = std::min(currentDualBound_, dualBound_);
#if ABC_NORMAL_DEBUG > 0
    ...
    default:
      ...
      break;
    case 1:
      currentDualTolerance_ = std::min(1.0e-1, dualTolerance_ * 10.0);
      stateDualColumn_ = -1;
      break;
    case 2:
      currentDualTolerance_ = std::min(1.0e-1, dualTolerance_ * 100.0);
      stateDualColumn_ = -1;
      break;
    case 3:
      currentDualTolerance_ = std::min(1.0e-1, dualTolerance_ * 10000.0);
      stateDualColumn_ = -1; // was -2 but that doesn't seem to work
      break;
    case 4:
      currentDualTolerance_ = std::min(1.0e-1, dualTolerance_ * 1000000.0);
      stateDualColumn_ = -1; // was -2 but that doesn't seem to work
      break;
    }
  } else {
  ...
  if (!problemStatus_) {
    ...
    double limit = 0.0;
    getDblParam(ClpDualObjectiveLimit, limit);
    if (fabs(limit) < 1.0e30 && minimizationObjectiveValue() > limit + 1.0e-7 + 1.0e-8 * fabs(limit)) {
  ...
  if (problemStatus_ == 3) {
    rawObjectiveValue_ = std::max(bestObjectiveValue_, rawObjectiveValue_ - bestPossibleImprovement_);
    setClpSimplexObjectiveValue();
  }
#if ABC_PARALLEL == 1
  if (parallelMode_ != 0) {
