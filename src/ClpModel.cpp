...
int ClpModel::loadProblem(CoinModel &modelObject, bool tryPlusMinusOne)
{
...
void ClpModel::gutsOfCopy(const ClpModel &rhs, int trueCopy)
{
  ...
  intParam_[ClpMaxNumIterationHotStart] = rhs.intParam_[ClpMaxNumIterationHotStart];
...
void ClpModel::copy(const ClpMatrixBase *from, ClpMatrixBase *&to)
{
  assert(from);
...
void ClpModel::borrowModel(ClpModel &rhs)
{
  if (defaultHandler_) {
    delete handler_;
    handler_ = NULL;
...
bool ClpModel::setIntParam(ClpIntParam key, int value)
{
  switch (key) {
...
bool ClpModel::setStrParam(ClpStrParam key, const std::string &value)
{
  ...
  bool createArray)
{
  if ((array || createArray) && size < newSize) {
    ...
    if (array)
      CoinMemcpyN(array, std::min(newSize, size), newArray);
    delete[] array;
    array = newArray;
...
// Resizes
void ClpModel::resize(int newNumberRows, int newNumberColumns)
{
  if (newNumberRows == numberRows_ && newNumberColumns == numberColumns_)
  ...
  if (status_) {
    if (newNumberColumns + newNumberRows) {
      if (newNumberColumns + newNumberRows > maximumRows_ + maximumColumns_) {
        ...
        CoinMemcpyN(status_, std::min(newNumberColumns, numberColumns_), tempC);
        CoinMemcpyN(status_ + numberColumns_, std::min(newNumberRows, numberRows_), tempR);
        delete[] status_;
        status_ = tempC;
  ...
  if (lengthNames_) {
    ...
    int numberRowNames = std::min(static_cast< int >(rowNames_.size()), numberRows_);
    if (numberRowNames < newNumberRows) {
      rowNames_.resize(newNumberRows);
      lengthNames_ = std::max(lengthNames_, 8);
      char name[10];
  ...
  if (integerType_ && numberColumns2 > maximumColumns_) {
    ...
    CoinMemcpyN(integerType_,
      std::min(newNumberColumns, numberColumns_), temp);
    delete[] integerType_;
    integerType_ = temp;
  }
  ...
  if (maximumRows_ >= 0) {
    ...
    maximumColumns_ = std::max(maximumColumns_, numberColumns_);
  }
}
...
{
  if (matrix_) {
    int numberRows = std::max(numberRows_,matrix_->getNumRows());
    int numberColumns = std::max(numberColumns_,matrix_->getNumCols());
  ...
  if (maximumRows_ < 0) {
    ...
    if (status_) {
      if (numberColumns_ + newSize) {
        unsigned char *tempR = reinterpret_cast< unsigned char * >(deleteChar(reinterpret_cast< char * >(status_) + numberColumns_,
          numberRows_,
...
// Deletes columns
void ClpModel::deleteColumns(int number, const int *which)
{
  if (!number)
    return; // nothing to do
  ...
  if (status_) {
    if (numberRows_ + newSize) {
      unsigned char *tempC = reinterpret_cast< unsigned char * >(deleteChar(reinterpret_cast< char * >(status_),
        numberColumns_,
        number, which, newSize, false));
...
void ClpModel::deleteRowsAndColumns(int numberRows, const int *whichRows,
...
int ClpModel::addRows(CoinModel &modelObject, bool tryPlusMinusOne, bool checkDuplicates)
{
    ...
    if ((!matrix_ || !matrix_->getNumElements()) && tryPlusMinusOne) {
      ...
      for (iColumn = 0; iColumn < number; iColumn++) {
        ...
        int numberElements = buildObject.column(iColumn, lower[iColumn],
          ...
          rows, elements);
        maximumLength = std::max(maximumLength, numberElements);
        for (int i = 0; i < numberElements; i++) {
          // allow for zero elements
          if (elements[i]) {
            if (fabs(elements[i]) == 1.0) {
...
int ClpModel::addColumns(CoinModel &modelObject, bool tryPlusMinusOne, bool checkDuplicates)
{
  if (modelObject.numberElements() == 0)
    return 0;
  bool goodState = true;
...
void ClpModel::onStopped()
{
  if (problemStatus_ == 3) {
    secondaryStatus_ = 0;
    if ((CoinCpuTime() >= dblParam_[ClpMaxSeconds] && dblParam_[ClpMaxSeconds] >= 0.0) || (CoinWallclockTime() >= dblParam_[ClpMaxWallSeconds] && dblParam_[ClpMaxWallSeconds] >= 0.0))
      secondaryStatus_ = 9;
  }
}
...
void ClpModel::popMessageHandler(CoinMessageHandler *oldHandler, bool oldDefault)
{
  if (defaultHandler_)
    delete handler_;
			  ...
			  double bigM)
{
  ...
  if (nExtra) {
    ...
    for (int iRow = 0; iRow < numberRows1; iRow++) {
      ...
      lengthNames_ =
	std::max(lengthNames_, static_cast<int>(strlen(name)));
      rowNames_.push_back(name);
      if ((rowType[iRow]&3)==3) {
	char name2[300];
	strcpy(name2,name);
	strcat(name2,"_fake_");
	lengthNames_ =
	  std::max(lengthNames_, static_cast<int>(strlen(name2)));
  ...
  for (int iTry=0;iTry<nTry;iTry++) 
  {
    ...
    double * solution = new double[numberColumns_];
    {
      ...
      double * temp = new double[std::max(numberRows,numberColumns_)];
      if (fp) {
	...
	for (int i=0;i<numberRows;i++) {
	  ...
	  if (value>rowUpper[i]+1.0e-8) {
	    nBad++;
	    worst = std::max(worst,value-rowUpper[i]);
	  } else if (value<rowLower[i]-1.0e-8) {
	    nBad++;
	    worst = std::max(worst,rowLower[i]-value);
	  }
	}
	...
	if ((iType&4)!=0) {
	  ...
	  rowUpper[iRow] = upper + std::max(newValue,0.0);
...
void ClpModel::replaceMatrix(ClpMatrixBase *matrix, bool deleteCurrent)
{
  if (deleteCurrent)
    delete matrix_;
  matrix_ = matrix;
  ...
  , savedColumnScale_(NULL)
{
  ...
  if (!dropNames) {
    ...
    for (iRow = 0; iRow < numberRows_; iRow++) {
      ...
      maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(rowNames_[iRow].c_str())));
    }
    int iColumn;
    columnNames_.reserve(numberColumns_);
    for (iColumn = 0; iColumn < numberColumns_; iColumn++) {
      columnNames_.push_back(rhs->columnNames_[whichColumn[iColumn]]);
      maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(columnNames_[iColumn].c_str())));
    }
    lengthNames_ = static_cast< int >(maxLength);
  } else {
    lengthNames_ = 0;
...
// Set row name
void ClpModel::setRowName(int iRow, std::string &name)
{
#ifndef NDEBUG
  if (iRow < 0 || iRow >= numberRows_) {
  ...
  rowNames_[iRow] = name;
  maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(name.c_str())));
...
// Set column name
void ClpModel::setColumnName(int iColumn, std::string &name)
{
#ifndef NDEBUG
  if (iColumn < 0 || iColumn >= numberColumns_) {
...
void ClpModel::copyRowNames(const std::vector< std::string > &rowNames, int first, int last)
{
  ...
  for (iRow = first; iRow < last; iRow++) {
    ...
    maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(rowNames_[iRow - first].c_str())));
  }
  // May be too big - but we would have to check both rows and columns to be exact
  lengthNames_ = static_cast< int >(maxLength);
}
// Copies in Column names - modifies names first .. last-1
void ClpModel::copyColumnNames(const std::vector< std::string > &columnNames, int first, int last)
{
  ...
  for (iColumn = first; iColumn < last; iColumn++) {
    ...
    maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(columnNames_[iColumn - first].c_str())));
  }
  // May be too big - but we would have to check both rows and columns to be exact
  lengthNames_ = static_cast< int >(maxLength);
}
// Copies in Row names - modifies names first .. last-1
void ClpModel::copyRowNames(const char *const *rowNames, int first, int last)
{
  // Do column names if necessary
  if (!lengthNames_ && numberColumns_) {
    lengthNames_ = 8;
  ...
  for (iRow = first; iRow < last; iRow++) {
    if (rowNames && rowNames[iRow - first] && strlen(rowNames[iRow - first])) {
      rowNames_[iRow] = rowNames[iRow - first];
      maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(rowNames[iRow - first])));
    } else {
      maxLength = std::max(maxLength, static_cast< unsigned int >(8));
      char name[10];
      sprintf(name, "R%7.7d", iRow);
      rowNames_[iRow] = name;
    }
...
// Copies in Column names - modifies names first .. last-1
void ClpModel::copyColumnNames(const char *const *columnNames, int first, int last)
{
  // Do row names if necessary
  if (!lengthNames_ && numberRows_) {
    lengthNames_ = 8;
  ...
  for (iColumn = first; iColumn < last; iColumn++) {
    if (columnNames && columnNames[iColumn - first] && strlen(columnNames[iColumn - first])) {
      columnNames_[iColumn] = columnNames[iColumn - first];
      maxLength = std::max(maxLength, static_cast< unsigned int >(strlen(columnNames[iColumn - first])));
    } else {
      maxLength = std::max(maxLength, static_cast< unsigned int >(8));
      char name[10];
      sprintf(name, "C%7.7d", iColumn);
      ...
      if (rowLower_[i] <= rowUpper_[i]) {
        if (rowLower_[i] > -1.0e30 || rowUpper_[i] < 1.0e30) {
          if (rowLower_[i] <= 0.0 && rowUpper_[i] >= 0.0) {
            ...
            sumPrimalInfeasibilities += std::min(rowLower_[i], -rowUpper_[i]);
            returnCode = 1;
          }
        } else {
          rowActivity_[i] = 0.0;
...
ClpModel::rowNamesAsChar() const
{
  ...
  if (lengthNames()) {
    ...
    numberNames = std::min(numberRows_, numberNames);
    unsigned int iRow;
...
ClpModel::columnNamesAsChar() const
{
  ...
  if (lengthNames()) {
    ...
    numberNames = std::min(numberColumns_, numberNames);
    unsigned int iColumn;
...
void ClpModel::deleteNamesAsChar(const char *const *names, int number) const
{
  for (int i = 0; i < number; i++) {
...
void ClpModel::scaling(int mode)
{
