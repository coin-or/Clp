...
*/
 ...
 /** Constructor */
ClpPESimplex::ClpPESimplex(ClpSimplex *model): coPrimalDegenerates_(0), primalDegenerates_(NULL), isPrimalDegenerate_(NULL),
coDualDegenerates_(0), dualDegenerates_(NULL), isDualDegenerate_(NULL),
coCompatibleCols_(0),  isCompatibleCol_(NULL), coCompatibleRows_(0),isCompatibleRow_(NULL),
model_(model), epsDegeneracy_(1.0e-07), epsCompatibility_(1.0e-07),
tempRandom_(NULL), 
...
ClpPESimplex::~ClpPESimplex() {
  ...
  // free memory
  if (primalDegenerates_)  free(primalDegenerates_);
  if (isPrimalDegenerate_) free(isPrimalDegenerate_);
  if (dualDegenerates_)  free(dualDegenerates_);
  if (isDualDegenerate_) free(isDualDegenerate_);
  if (isCompatibleCol_) free(isCompatibleCol_);
  if (compatibilityCol_) free(compatibilityCol_);
  if (isCompatibleRow_) free(isCompatibleRow_);
  if (compatibilityRow_) free(compatibilityRow_);
  if (tempRandom_)      free(tempRandom_);
...
void ClpPESimplex::updatePrimalDegeneratesAvg(int coPivots){
	...
	coPrimalDegeneratesAvg_ = floor( (1.0-fracPivots)* 
					 (coPrimalDegeneratesAvg_ + fracPivots*coPrimalDegenerates_));
}
