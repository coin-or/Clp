...
: ClpDualRowSteepest(mode), modelPE_(NULL), psi_(psi),
...
bool checkCompatibles = true;
 double ratioCompatibles = static_cast<double> ( modelPE_->coCompatibleRows())/
   static_cast<double> (std::min(model_->numberRows(),model_->numberColumns()));
...
else modelPE_->isLastPivotCompatible(false);
...
*/
void 
ClpPEDualRowSteepest::updatePrimalSolution(CoinIndexedVector * input,
					   double theta,
