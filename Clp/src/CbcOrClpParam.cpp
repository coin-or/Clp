...
{
  numberParameters = 0;
  parameters[numberParameters++] = CbcOrClpParam("?", "For help", CBC_PARAM_GENERALQUERY, 7, 0);
  parameters[numberParameters++] = CbcOrClpParam("???", "For help", CBC_PARAM_FULLGENERALQUERY, 7, 0);
  parameters[numberParameters++] = CbcOrClpParam("-", "From stdin",
    CLP_PARAM_ACTION_STDIN, 3, 0);
#if defined(ABC_INHERIT) || ABOCA_LITE
  parameters[numberParameters++] = CbcOrClpParam("abc", "Whether to visit Aboca",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("allC!ommands", "Whether to print less used commands",
...
are printed out on ?.");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("allow!ableGap", "Stop when gap between best possible and \
best less than this",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("allS!lack", "Set basis back to all slack and reset solution",
...
basis anyway.");
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("artif!icialCost", "Costs >= this treated as artificials in feasibility pump",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("auto!Scale", "Whether to scale objective, rhs and bounds of problem if they look odd",
 ...
 to use objective!Scale and rhs!Scale.");
  parameters[numberParameters++] = CbcOrClpParam("barr!ier", "Solve using primal dual predictor corrector algorithm",
...
with quadratic objectives."
  ...
  parameters[numberParameters++] = CbcOrClpParam("basisI!n", "Import basis from bas file",
 ...
 files 'xxxxxxxx.gz' or xxxxxxxx.bz2.");
  parameters[numberParameters++] = CbcOrClpParam("basisO!ut", "Export basis as bas file",
 ...
 is initialized to 'default.bas'.");
  parameters[numberParameters++] = CbcOrClpParam("biasLU", "Whether factorization biased towards U",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("branch!AndCut", "Do Branch and Cut",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("bscale", "Whether to scale in barrier (and ordering speed)",
...
#if FLUSH_PRINT_BUFFER > 2
  parameters[numberParameters++] = CbcOrClpParam("buff!eredMode", "Whether to flush print buffer",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("chol!esky", "Which cholesky algorithm",
...
possibilities.");
  //#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("clique!Cuts", "Whether to use Clique cuts",
...
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("combine!Solutions", "Whether to use combine solution heuristic",
...
See Rounding for meaning of on,both,before");
  parameters[numberParameters++] = CbcOrClpParam("combine2!Solutions", "Whether to use crossover solution heuristic",
...
It obviously only tries after two or more solutions. \
See Rounding for meaning of on,both,before");
  parameters[numberParameters++] = CbcOrClpParam("constraint!fromCutoff", "Whether to use cutoff as constraint",
  ...
  parameters[numberParameters++] = CbcOrClpParam("cost!Strategy", "How to use costs as priorities",
...
first.  This primitive strategy can be surprsingly effective.  The column order\
 option is obviously not on costs but easy to code here.");
  parameters[numberParameters++] = CbcOrClpParam("cplex!Use", "Whether to use Cplex!",
...
both.");
#endif
  parameters[numberParameters++] = CbcOrClpParam("cpp!Generate", "Generates C++ code",
...
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("crash", "Whether to create basis for problem",
...
I have also added a variant due to Solow and Halim which is as on but just flip.");
  parameters[numberParameters++] = CbcOrClpParam("cross!over", "Whether to get a basic solution after barrier",
...
presolve as well) - the option maybe does this.");
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("csv!Statistics", "Create one line of statistics",
 ...
 is initialized to '', i.e. it must be set.  Adds header if file empty or does not exist.");
  parameters[numberParameters++] = CbcOrClpParam("cutD!epth", "Depth in tree at which to do cuts",
...
to -1 (off -> code decides).");
  ...
  parameters[numberParameters++] = CbcOrClpParam("cutL!ength", "Length of a cut",
...
at root and 100 in tree.");
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("cuto!ff", "All solutions must be better than this",
...
objective for solution minus cutoff increment.");
  parameters[numberParameters++] = CbcOrClpParam("cuts!OnOff", "Switches all cuts on or off",
...
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("debug!In", "read valid solution from file",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("decomp!ose", "Whether to try decomposition",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("depth!MiniBab", "Depth at which to try mini BAB",
...
means use Cplex if it is linked in.  Otherwise if negative then go into depth first complete search fast branch and bound when depth>= -value-2 (so -3 will use this at depth>=1).  This mode is only switched on after 500 nodes.  If you really want to switch it off for small problems then set this to -999.  If >=0 the value doesn't matter very much.  The code will do approximately 100 nodes of fast branch and bound every now and then at depth>=5.  The actual logic is too twisted to describe here.");
  parameters[numberParameters++] = CbcOrClpParam("dextra3", "Extra double parameter 3",
    -1.0e100, 1.0e100, CBC_PARAM_DBL_DEXTRA3, 0);
  parameters[numberParameters - 1].setDoubleValue(0.0);
  parameters[numberParameters++] = CbcOrClpParam("dextra4", "Extra double parameter 4",
    -1.0e100, 1.0e100, CBC_PARAM_DBL_DEXTRA4, 0);
  parameters[numberParameters - 1].setDoubleValue(0.0);
  parameters[numberParameters++] = CbcOrClpParam("dextra5", "Extra double parameter 5",
    -1.0e100, 1.0e100, CBC_PARAM_DBL_DEXTRA5, 0);
  parameters[numberParameters - 1].setDoubleValue(0.0);
  parameters[numberParameters++] = CbcOrClpParam("Dins", "Whether to try Distance Induced Neighborhood Search",
...
See Rounding for meaning of on,both,before");
#endif
  parameters[numberParameters++] = CbcOrClpParam("direction", "Minimize or Maximize",
...
You can also use the parameters 'maximize' or 'minimize'.");
  parameters[numberParameters++] = CbcOrClpParam("directory", "Set Default directory for import etc.",
    CLP_PARAM_ACTION_DIRECTORY);
  parameters[numberParameters - 1].setLonghelp(
    "This sets the directory which import, export, saveModel, restoreModel etc will use.\
  It is initialized to './'");
  parameters[numberParameters++] = CbcOrClpParam("dirSample", "Set directory where the COIN-OR sample problems are.",
 ...
 It is initialized to '../../Data/Sample'");
  parameters[numberParameters++] = CbcOrClpParam("dirNetlib", "Set directory where the netlib problems are.",
 ...
 It is initialized to '../../Data/Netlib'");
  parameters[numberParameters++] = CbcOrClpParam("dirMiplib", "Set directory where the miplib 2003 problems are.",
 ...
 It is initialized to '../../Data/miplib3'");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("diveO!pt", "Diving options",
  ...
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("diveS!olves", "Diving solve option",
...
and used for extra options - \
  ...
  parameters[numberParameters++] = CbcOrClpParam("DivingS!ome", "Whether to try Diving heuristics",
...
See Rounding for meaning of on,both,before");
  parameters[numberParameters++] = CbcOrClpParam("DivingC!oefficient", "Whether to try DiveCoefficient",
    "off", CBC_PARAM_STR_DIVINGC);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].append("both");
  parameters[numberParameters - 1].append("before");
  parameters[numberParameters++] = CbcOrClpParam("DivingF!ractional", "Whether to try DiveFractional",
    "off", CBC_PARAM_STR_DIVINGF);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].append("both");
  parameters[numberParameters - 1].append("before");
  parameters[numberParameters++] = CbcOrClpParam("DivingG!uided", "Whether to try DiveGuided",
    "off", CBC_PARAM_STR_DIVINGG);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].append("both");
  parameters[numberParameters - 1].append("before");
  parameters[numberParameters++] = CbcOrClpParam("DivingL!ineSearch", "Whether to try DiveLineSearch",
    "off", CBC_PARAM_STR_DIVINGL);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].append("both");
  parameters[numberParameters - 1].append("before");
  parameters[numberParameters++] = CbcOrClpParam("DivingP!seudoCost", "Whether to try DivePseudoCost",
    "off", CBC_PARAM_STR_DIVINGP);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].append("both");
  parameters[numberParameters - 1].append("before");
  parameters[numberParameters++] = CbcOrClpParam("DivingV!ectorLength", "Whether to try DiveVectorLength",
    "off", CBC_PARAM_STR_DIVINGV);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].append("both");
  parameters[numberParameters - 1].append("before");
  parameters[numberParameters++] = CbcOrClpParam("doH!euristic", "Do heuristics before any preprocessing",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("dualB!ound", "Initially algorithm acts as if no \
gap between bounds exceeds this value",
 ...
 adjust bounds, maybe we need that here.");
  parameters[numberParameters++] = CbcOrClpParam("dualize", "Solves dual reformulation",
    0, 4, CLP_PARAM_INT_DUALIZE, 1);
  parameters[numberParameters - 1].setLonghelp(
    "Don't even think about it.");
  parameters[numberParameters++] = CbcOrClpParam("dualP!ivot", "Dual pivot choice algorithm",
...
This selects incoming variables to try and avoid degenerate moves. See definition of psi.");
  parameters[numberParameters++] = CbcOrClpParam("dualS!implex", "Do dual simplex algorithm",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("dualT!olerance", "For an optimal solution \
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("dw!Heuristic", "Whether to try DW heuristic",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("either!Simplex", "Do dual or primal simplex algorithm",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("end", "Stops clp execution",
    CLP_PARAM_ACTION_EXIT);
  parameters[numberParameters - 1].setLonghelp(
    "This stops execution ; end, exit, quit and stop are synonyms");
  parameters[numberParameters++] = CbcOrClpParam("environ!ment", "Read commands from environment",
    CLP_PARAM_ACTION_ENVIRONMENT, 7, 0);
  parameters[numberParameters - 1].setLonghelp(
    "This starts reading from environment variable CBC_CLP_ENVIRONMENT.");
  parameters[numberParameters++] = CbcOrClpParam("error!sAllowed", "Whether to allow import errors",
...
e.g. no ENDATA.  This has to be set before import i.e. -errorsAllowed on -import xxxxxx.mps.");
  parameters[numberParameters++] = CbcOrClpParam("exit", "Stops clp execution",
    CLP_PARAM_ACTION_EXIT);
  parameters[numberParameters - 1].setLonghelp(
    "This stops the execution of Clp, end, exit, quit and stop are synonyms");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("exper!iment", "Whether to use testing features",
...
0 then no new ideas, 1 fairly sensible, 2 a bit dubious, 3 you are on your own!");
  parameters[numberParameters - 1].setIntValue(0);
  parameters[numberParameters++] = CbcOrClpParam("expensive!Strong", "Whether to do even more strong branching",
...
\n\tIf >=100 then do when depth <= strategy/100 (otherwise 5)");
  parameters[numberParameters - 1].setIntValue(0);
#endif
  parameters[numberParameters++] = CbcOrClpParam("export", "Export model as mps file",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("extra1", "Extra integer parameter 1",
    -COIN_INT_MAX, COIN_INT_MAX, CBC_PARAM_INT_EXTRA1, 0);
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("extra2", "Extra integer parameter 2",
    -100, COIN_INT_MAX, CBC_PARAM_INT_EXTRA2, 0);
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("extra3", "Extra integer parameter 3",
    -1, COIN_INT_MAX, CBC_PARAM_INT_EXTRA3, 0);
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("extra4", "Extra integer parameter 4",
...
\n\t16000 - switches on alternate gomory criterion");
  parameters[numberParameters++] = CbcOrClpParam("extraV!ariables", "Allow creation of extra integer variables",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("fact!orization", "Which factorization to use",
...
a dense one, one designed for small problems or if enabled a long factorization."
  ...
  parameters[numberParameters++] = CbcOrClpParam("fakeB!ound", "All bounds <= this value - DEBUG",
    1.0, 1.0e15, CLP_PARAM_ACTION_FAKEBOUND, 0);
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("feas!ibilityPump", "Whether to try Feasibility Pump",
...
See Rounding for meaning of on,both,before");
  parameters[numberParameters++] = CbcOrClpParam("fix!OnDj", "Try heuristic based on fixing variables with \
...
before branch and bound - use with extreme caution!");
  parameters[numberParameters++] = CbcOrClpParam("flow!CoverCuts", "Whether to use Flow Cover cuts",
...
Can also enter testing values by plusnn (==ifmove)");
  parameters[numberParameters++] = CbcOrClpParam("force!Solution", "Whether to use given solution as crash for BAB",
...
If >1 then also does that many nodes on fixed problem.");
  parameters[numberParameters++] = CbcOrClpParam("fraction!forBAB", "Fraction in feasibility pump",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("gamma!(Delta)", "Whether to regularize barrier",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("GMI!Cuts", "Whether to use alternative Gomory cuts",
...
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("gomory!Cuts", "Whether to use Gomory cuts",
...
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("greedy!Heuristic", "Whether to use a greedy heuristic",
...
See Rounding for meaning of on,both,before");
#endif
  parameters[numberParameters++] = CbcOrClpParam("gsolu!tion", "Puts glpk solution to file",
...
If problem created from gmpl model - will do any reports.");
  parameters[numberParameters++] = CbcOrClpParam("guess", "Guesses at good parameters",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("heur!isticsOnOff", "Switches most heuristics on or off",
...
alters search.");
#endif
  parameters[numberParameters++] = CbcOrClpParam("help", "Print out version, non-standard options and some help",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("hOp!tions", "Heuristic options",
...
while 8 will adapt cutoff rhs after first solution if it looks as if code is stalling.");
  parameters[numberParameters - 1].setIntValue(0);
  parameters[numberParameters++] = CbcOrClpParam("hot!StartMaxIts", "Maximum iterations on hot start",
    0, COIN_INT_MAX, CBC_PARAM_INT_MAXHOTITS);
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("idiot!Crash", "Whether to try idiot crash",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("import", "Import model from mps file",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("inc!rement", "A valid solution must be at least this \
...
objective then this can be set to 0.01.  Be careful if you set this negative!");
  parameters[numberParameters++] = CbcOrClpParam("inf!easibilityWeight", "Each integer infeasibility is expected \
to cost this much",
...
expected to cost this much.");
  parameters[numberParameters++] = CbcOrClpParam("initialS!olve", "Solve to continuous",
    CLP_PARAM_ACTION_SOLVECONTINUOUS);
  parameters[numberParameters - 1].setLonghelp(
    "This just solves the problem to continuous - without adding any cuts");
  parameters[numberParameters++] = CbcOrClpParam("integerT!olerance", "For an optimal solution \
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("keepN!ames", "Whether to keep names from import",
...
This needs to be set before the import of model - so -keepnames off -import xxxxx.mps.");
  parameters[numberParameters++] = CbcOrClpParam("KKT", "Whether to use KKT factorization",
    "off", CLP_PARAM_STR_KKT, 7, 1);
  parameters[numberParameters - 1].append("on");
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("knapsack!Cuts", "Whether to use Knapsack cuts",
...
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("lagomory!Cuts", "Whether to use Lagrangean Gomory cuts",
...
The length options for gomory cuts are used.");
  parameters[numberParameters++] = CbcOrClpParam("latwomir!Cuts", "Whether to use Lagrangean TwoMir cuts",
...
lagomoryCuts for description of options.");
  parameters[numberParameters++] = CbcOrClpParam("lift!AndProjectCuts", "Whether to use Lift and Project cuts",
...
May be slow \
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("local!TreeSearch", "Whether to use local treesearch",
...
heuristics are switched on.");
#endif
#ifndef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("log!Level", "Level of detail in Solver output",
    -1, 999999, CLP_PARAM_INT_SOLVERLOGLEVEL);
#else
  parameters[numberParameters++] = CbcOrClpParam("log!Level", "Level of detail in Coin branch and Cut output",
 ...
 value for most uses, while 2 and 3 give more information.");
  parameters[numberParameters++] = CbcOrClpParam("max!imize", "Set optimization direction to maximize",
...
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("maxF!actor", "Maximum number of iterations between \
refactorizations",
 ...
 earlier for accuracy.");
  parameters[numberParameters++] = CbcOrClpParam("maxIt!erations", "Maximum number of iterations before \
stopping",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("maxN!odes", "Maximum number of nodes to do",
...
but then the results may not be repeatable.");
  parameters[numberParameters++] = CbcOrClpParam("maxSaved!Solutions", "Maximum number of solutions to save",
    0, 2147483647, CBC_PARAM_INT_MAXSAVEDSOLS);
  parameters[numberParameters - 1].setLonghelp(
    "Number of solutions to save.");
  parameters[numberParameters++] = CbcOrClpParam("maxSo!lutions", "Maximum number of solutions to get",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("min!imize", "Set optimization direction to minimize",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("mipO!ptions", "Dubious options for mip",
    0, COIN_INT_MAX, CBC_PARAM_INT_MIPOPTIONS, 0);
  parameters[numberParameters++] = CbcOrClpParam("more!MipOptions", "More dubious options for mip",
    -1, COIN_INT_MAX, CBC_PARAM_INT_MOREMIPOPTIONS, 0);
  parameters[numberParameters++] = CbcOrClpParam("more2!MipOptions", "More more dubious options for mip",
    -1, COIN_INT_MAX, CBC_PARAM_INT_MOREMOREMIPOPTIONS, 0);
  parameters[numberParameters - 1].setIntValue(0);
  parameters[numberParameters++] = CbcOrClpParam("mixed!IntegerRoundingCuts", "Whether to use Mixed Integer Rounding cuts",
...
See branchAndCut for information on options.");
#endif
  parameters[numberParameters++] = CbcOrClpParam("mess!ages", "Controls if Clpnnnn is printed",
    "off", CLP_PARAM_STR_MESSAGES);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].setLonghelp("The default behavior is to put out messages such as:\n\
...
but this program turns this off to make it look more friendly.  It can be useful\
 ...
 you intend to override the behavior of a particular message.  This only affects Clp not Cbc.");
  parameters[numberParameters++] = CbcOrClpParam("miplib", "Do some of miplib test set",
    CBC_PARAM_ACTION_MIPLIB, 3, 1);
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("mips!tart", "reads an initial feasible solution from file",
...
");
#endif
  parameters[numberParameters++] = CbcOrClpParam("moreS!pecialOptions", "Yet more dubious options for Simplex - see ClpSimplex.hpp",
    0, COIN_INT_MAX, CLP_PARAM_INT_MORESPECIALOPTIONS, 0);
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("moreT!une", "Yet more dubious ideas for feasibility pump",
...
\t n fix if value of variable same for last n iterations.");
  parameters[numberParameters - 1].setIntValue(0);
  parameters[numberParameters++] = CbcOrClpParam("multiple!RootPasses", "Do multiple root passes to collect cuts and solutions",
...
then cuts are collected and used in later passes - so there is interaction there.");
  parameters[numberParameters++] = CbcOrClpParam("naive!Heuristics", "Whether to try some stupid heuristic",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("netlib", "Solve entire netlib test set",
...
The user can set options before e.g. clp -presolve off -netlib");
  parameters[numberParameters++] = CbcOrClpParam("netlibB!arrier", "Solve entire netlib test set with barrier",
...
The user can set options before e.g. clp -kkt on -netlib");
  parameters[numberParameters++] = CbcOrClpParam("netlibD!ual", "Solve entire netlib test set (dual)",
    CLP_PARAM_ACTION_NETLIB_DUAL, 3, 1);
  parameters[numberParameters - 1].setLonghelp(
    "This exercises the unit test for clp and then solves the netlib test set using dual.\
The user can set options before e.g. clp -presolve off -netlib");
  parameters[numberParameters++] = CbcOrClpParam("netlibP!rimal", "Solve entire netlib test set (primal)",
...
The user can set options before e.g. clp -presolve off -netlibp");
  parameters[numberParameters++] = CbcOrClpParam("netlibT!une", "Solve entire netlib test set with 'best' algorithm",
...
with University of Florida ordering.");
  parameters[numberParameters++] = CbcOrClpParam("network", "Tries to make network matrix",
...
specialized network code.");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("nextB!estSolution", "Prints next best saved solution to file",
 ...
 is initialized to 'stdout'.  The amount of output can be varied using printi!ngOptions or printMask.");
  parameters[numberParameters++] = CbcOrClpParam("node!Strategy", "What strategy to use to select nodes",
...
Default has now been changed to hybrid which is breadth first on small depth nodes then fewest.");
  parameters[numberParameters++] = CbcOrClpParam("numberA!nalyze", "Number of analysis iterations",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("objective!Scale", "Scale factor to apply to objective",
...
#ifdef COIN_HAS_CBC
#ifdef COIN_HAS_NTY
  parameters[numberParameters++] = CbcOrClpParam("Orbit!alBranching", "Whether to try orbital branching",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("PrepN!ames", "If column names will be kept in pre-processed model",
...
Setting this option to on keeps original names in variables which still exist in the preprocessed problem");
  ...
  parameters[numberParameters++] = CbcOrClpParam("outDup!licates", "takes duplicate rows etc out of integer model",
    CLP_PARAM_ACTION_OUTDUPROWS, 7, 0);
#endif
  parameters[numberParameters++] = CbcOrClpParam("output!Format", "Which output format to use",
...
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("para!metrics", "Import data from file and do parametrics",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("passC!uts", "Number of cut passes at root node",
...
stop if drop small if less than 5000 columns, 20 otherwise");
  parameters[numberParameters++] = CbcOrClpParam("passF!easibilityPump", "How many passes in feasibility pump",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("passP!resolve", "How many passes in presolve",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("passT!reeCuts", "Number of cut passes in tree",
    -9999999, 9999999, CBC_PARAM_INT_CUTPASSINTREE);
  parameters[numberParameters - 1].setLonghelp(
    "The default is one pass");
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("pertV!alue", "Method of perturbation",
    -5000, 102, CLP_PARAM_INT_PERTVALUE, 1);
  parameters[numberParameters++] = CbcOrClpParam("perturb!ation", "Whether to perturb problem",
 ...
 benefit from perturbation.  Normally Clp tries to be intelligent, but you can switch this off.\
  ...
  parameters[numberParameters++] = CbcOrClpParam("PFI", "Whether to use Product Form of Inverse in simplex",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("pivotAndC!omplement", "Whether to try Pivot and Complement heuristic",
  ...
  parameters[numberParameters - 1].setLonghelp(
    "stuff needed. \
Doh option does heuristic before preprocessing");
  parameters[numberParameters++] = CbcOrClpParam("pivotAndF!ix", "Whether to try Pivot and Fix heuristic",
...
Doh option does heuristic before preprocessing");
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("plus!Minus", "Tries to make +- 1 matrix",
 ...
 see if it can convert the problem so you should not need to use this.");
  parameters[numberParameters++] = CbcOrClpParam("pO!ptions", "Dubious print options",
  ...
  parameters[numberParameters++] = CbcOrClpParam("preO!pt", "Presolve options",
    0, COIN_INT_MAX, CLP_PARAM_INT_PRESOLVEOPTIONS, 0);
#endif
  parameters[numberParameters++] = CbcOrClpParam("presolve", "Whether to presolve problem",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("preprocess", "Whether to use integer preprocessing",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("preT!olerance", "Tolerance to use in presolve",
...
infeasible and you have awkward numbers and you are sure the problem is really feasible.");
  parameters[numberParameters++] = CbcOrClpParam("primalP!ivot", "Primal pivot choice algorithm",
...
Omer, J., Towhidi, M., Soumis, F., The positive edge pricing rule for the dual simplex.");
  parameters[numberParameters++] = CbcOrClpParam("primalS!implex", "Do primal simplex algorithm",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("primalT!olerance", "For an optimal solution \
...
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("primalW!eight", "Initially algorithm acts as if it \
costs this much to be infeasible",
 ...
 adjust bounds, maybe we need that here.");
  parameters[numberParameters++] = CbcOrClpParam("psi", "Two-dimension pricing factor for Positive edge",
...
Omer, J., Towhidi, M., Soumis, F., The positive edge pricing rule for the dual simplex.");
#endif
  parameters[numberParameters++] = CbcOrClpParam("printi!ngOptions", "Print options",
...
Also see printMask for controlling output.");
  parameters[numberParameters++] = CbcOrClpParam("printM!ask", "Control printing of solution on a  mask",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("prio!rityIn", "Import priorities etc from file",
...
File is in csv format with allowed headings - name, number, priority, direction, up, down, solution.  Exactly one of\
 name and number must be given.");
  parameters[numberParameters++] = CbcOrClpParam("probing!Cuts", "Whether to use Probing cuts",
...
but strong options do more probing");
  parameters[numberParameters++] = CbcOrClpParam("proximity!Search", "Whether to do proximity search heuristic",
...
Can also set different maxNode settings by plusnnnn (and are 'on'(on==30)).");
  parameters[numberParameters++] = CbcOrClpParam("pumpC!utoff", "Fake cutoff for use in feasibility pump",
    -1.0e100, 1.0e100, CBC_PARAM_DBL_FAKECUTOFF);
  parameters[numberParameters - 1].setDoubleValue(0.0);
  parameters[numberParameters - 1].setLonghelp(
    "0.0 off - otherwise add a constraint forcing objective below this value\
 in feasibility pump");
  parameters[numberParameters++] = CbcOrClpParam("pumpI!ncrement", "Fake increment for use in feasibility pump",
...
when solution found in feasibility pump");
  parameters[numberParameters++] = CbcOrClpParam("pumpT!une", "Dubious ideas for feasibility pump",
...
are fixed and a small branch and bound is tried.");
  parameters[numberParameters - 1].setIntValue(0);
#endif
  parameters[numberParameters++] = CbcOrClpParam("quit", "Stops clp execution",
    CLP_PARAM_ACTION_EXIT);
  parameters[numberParameters - 1].setLonghelp(
    "This stops the execution of Clp, end, exit, quit and stop are synonyms");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("randomC!bcSeed", "Random seed for Cbc",
    -1, COIN_INT_MAX, CBC_PARAM_INT_RANDOMSEED);
  parameters[numberParameters - 1].setLonghelp(
    "This sets a random seed for Cbc \
- 0 says use time of day, -1 is as now.");
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("randomi!zedRounding", "Whether to try randomized rounding heuristic",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("randomS!eed", "Random seed for Clp",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("ratio!Gap", "Stop when gap between best possible and \
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("restoreS!olution", "reads solution from file",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("readSt!ored", "Import stored cuts from file",
    CLP_PARAM_ACTION_STOREDFILE, 3, 0);
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("reallyO!bjectiveScale", "Scale factor to apply to objective in place",
    -1.0e20, 1.0e20, CLP_PARAM_DBL_OBJSCALE2, 0);
  parameters[numberParameters - 1].setLonghelp(
    "You can set this to -1.0 to test maximization or other to stress code");
  parameters[numberParameters - 1].setDoubleValue(1.0);
  parameters[numberParameters++] = CbcOrClpParam("reallyS!cale", "Scales model in place",
    CLP_PARAM_ACTION_REALLY_SCALE, 7, 0);
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("reduce!AndSplitCuts", "Whether to use Reduce-and-Split cuts",
...
May be slow \
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("reduce2!AndSplitCuts", "Whether to use Reduce-and-Split cuts - style 2",
...
Standard setting only uses rows in tableau <=256, long uses all \
See branchAndCut for information on options.");
  parameters[numberParameters++] = CbcOrClpParam("residual!CapacityCuts", "Whether to use Residual Capacity cuts",
...
See branchAndCut for information on options.");
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("restore!Model", "Restore model from binary file",
 ...
 is initialized to 'default.prob'.");
  parameters[numberParameters++] = CbcOrClpParam("reverse", "Reverses sign of objective",
    CLP_PARAM_ACTION_REVERSE, 7, 0);
  parameters[numberParameters - 1].setLonghelp(
    "Useful for testing if maximization works correctly");
  parameters[numberParameters++] = CbcOrClpParam("rhs!Scale", "Scale factor to apply to rhs and bounds",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("Rens", "Whether to try Relaxation Enforced Neighborhood Search",
...
Doh option does heuristic before preprocessing");
  parameters[numberParameters++] = CbcOrClpParam("Rins", "Whether to try Relaxed Induced Neighborhood Search",
...
Doh option does heuristic before preprocessing");
  parameters[numberParameters++] = CbcOrClpParam("round!ingHeuristic", "Whether to use Rounding heuristic",
...
and both means do if doHeuristics and in solve.");
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("saveM!odel", "Save model to binary file",
 ...
 directory given by 'directory'.  A name of '$' will use the previous value for the name.  This\
 is initialized to 'default.prob'.");
  parameters[numberParameters++] = CbcOrClpParam("saveS!olution", "saves solution to file",
...
If name contains '_fix_read_' then does not write but reads and will fix all variables");
  parameters[numberParameters++] = CbcOrClpParam("scal!ing", "Whether to scale problem",
...
#ifndef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("sec!onds", "Maximum seconds",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("sleep", "for debug",
    CLP_PARAM_ACTION_DUMMY, 7, 0);
  parameters[numberParameters - 1].setLonghelp(
    "If passed to solver fom ampl, then ampl will wait so that you can copy .nl file for debug.");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("slow!cutpasses", "Maximum number of tries for slower cuts",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("slp!Value", "Number of slp passes before primal",
...
#if CLP_MULTIPLE_FACTORIZATIONS > 0
  parameters[numberParameters++] = CbcOrClpParam("small!Factorization", "Whether to use small factorization",
...
#endif
#endif
  parameters[numberParameters++] = CbcOrClpParam("solu!tion", "Prints solution to file",
...
#ifdef COIN_HAS_CLP
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("solv!e", "Solve problem",
    CBC_PARAM_ACTION_BAB);
  parameters[numberParameters - 1].setLonghelp(
...
a low priority.  Orderlow gives integers high priority then SOS in order.");
 ...
 value for most uses, while 2 and 3 give more information.  This parameter is only used inside MIP - for Clp use 'log'");
  ...
  parameters[numberParameters++] = CbcOrClpParam("solv!e", "Solve problem using dual simplex (probably)",
    CLP_PARAM_ACTION_EITHERSIMPLEX);
  parameters[numberParameters - 1].setLonghelp(
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("spars!eFactor", "Whether factorization treated as sparse",
    "on", CLP_PARAM_STR_SPARSEFACTOR, 7, 0);
  parameters[numberParameters - 1].append("off");
  parameters[numberParameters++] = CbcOrClpParam("special!Options", "Dubious options for Simplex - see ClpSimplex.hpp",
    0, COIN_INT_MAX, CLP_PARAM_INT_SPECIALOPTIONS, 0);
  parameters[numberParameters++] = CbcOrClpParam("sprint!Crash", "Whether to try sprint crash",
 ...
 an LP code of that name of the 60's which tried the same tactic (not totally successfully).\
  ...
  parameters[numberParameters++] = CbcOrClpParam("stat!istics", "Print some statistics",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("stop", "Stops clp execution",
    CLP_PARAM_ACTION_EXIT);
  parameters[numberParameters - 1].setLonghelp(
    "This stops the execution of Clp, end, exit, quit and stop are synonyms");
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("strat!egy", "Switches on groups of features",
...
This does not apply to unit tests (where 'experiment' may have similar effects).");
  ...
  parameters[numberParameters++] = CbcOrClpParam("strengthen", "Create strengthened problem",
...
#endif
  parameters[numberParameters++] = CbcOrClpParam("strong!Branching", "Number of variables to look at in strong branching",
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("subs!titution", "How long a column to substitute for in presolve",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("testO!si", "Test OsiObject stuff",
    -1, COIN_INT_MAX, CBC_PARAM_INT_TESTOSI, 0);
#endif
#ifdef CBC_THREAD
  parameters[numberParameters++] = CbcOrClpParam("thread!s", "Number of threads to try and use",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("tighten!Factor", "Tighten bounds using this times largest \
...
#endif
#ifdef COIN_HAS_CLP
  parameters[numberParameters++] = CbcOrClpParam("tightLP", "Poor person's preSolve for now",
    CLP_PARAM_ACTION_TIGHTEN, 7, 0);
#endif
  parameters[numberParameters++] = CbcOrClpParam("timeM!ode", "Whether to use CPU or elapsed time",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("trust!PseudoCosts", "Number of branches before we trust pseudocosts",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("tune!PreProcess", "Dubious tuning parameters",
...
12 - 4096 Switch off duplicate column checking for integers");
  parameters[numberParameters++] = CbcOrClpParam("two!MirCuts", "Whether to use Two phase Mixed Integer Rounding cuts",
...
See branchAndCut for information on options.");
#endif
  parameters[numberParameters++] = CbcOrClpParam("unitTest", "Do unit test",
    CLP_PARAM_ACTION_UNITTEST, 3, 1);
  parameters[numberParameters - 1].setLonghelp(
    "This exercises the unit test for clp");
  parameters[numberParameters++] = CbcOrClpParam("userClp", "Hand coded Clp stuff",
...
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("userCbc", "Hand coded Cbc stuff",
...
#endif
#ifdef COIN_AVX2
  parameters[numberParameters++] = CbcOrClpParam("vector!Mode", "Try and use vector instructions",
...
#endif
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("Vnd!VariableNeighborhoodSearch", "Whether to try Variable Neighborhood Search",
...
Doh option does heuristic before preprocessing");
#endif
  parameters[numberParameters++] = CbcOrClpParam("vector", "Whether to use vector? Form of matrix in simplex",
    "off", CLP_PARAM_STR_VECTOR, 7, 0);
  parameters[numberParameters - 1].append("on");
  parameters[numberParameters - 1].setLonghelp(
    "If this is on ClpPackedMatrix uses extra column copy in odd format.");
  parameters[numberParameters++] = CbcOrClpParam("verbose", "Switches on longer help on single ?",
  ...
  parameters[numberParameters - 1].setIntValue(0);
#ifdef COIN_HAS_CBC
  parameters[numberParameters++] = CbcOrClpParam("vub!heuristic", "Type of vub heuristic",
    -2, 20, CBC_PARAM_INT_VUBTRY, 0);
  parameters[numberParameters - 1].setLonghelp(
    "If set will try and fix some integer variables");
  parameters[numberParameters - 1].setIntValue(-1);
  parameters[numberParameters++] = CbcOrClpParam("zero!HalfCuts", "Whether to use zero half cuts",
...
Alberto Caprara.");
#endif
  parameters[numberParameters++] = CbcOrClpParam("zeroT!olerance", "Kill all coefficients \
