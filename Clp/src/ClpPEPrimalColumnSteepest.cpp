...
// Local copy of mode so can decide what to do
	int switchType;
	if (mode_ == 4) switchType = 5 - numberSwitched_;
	else if (mode_ >= 10) switchType = 3;
	else switchType = mode_;
	...
	*/
	...
	if (infeas[sequenceOut]) infeas[sequenceOut] = value * value; // already there
	else infeasible_->quickAdd(sequenceOut, value * value);
	} else {
	...
	*/
	...
	if (modelPE_->coCompatibleCols()*1.0>model_->numberColumns()) {
	  ...
	  psi_ = 1.0;
			...
			if (iCurrent_ == iInterval_) iInterval_ = std::max(50, iInterval_-50);
			else								  iInterval_ = std::min(300, iInterval_+50);
	...
	** now do the pricing */
	...
	double ratioCompatibles = static_cast<double> ( modelPE_->coCompatibleCols())/
	  static_cast<double> ((model_->numberRows()+model_->numberColumns()));
	double ratioCompatibles2 = static_cast<double> ( modelPE_->coCompatibleCols())/
	  static_cast<double> (model_->numberColumns());

	if (psi_ >= 1.0 || ratioCompatibles < 0.01)	checkCompatibles = false;
	if (ratioCompatibles2>0.5) checkCompatibles=false;
	...
	if (bestSequence>=0&&model_->getStatus(bestSequence)==
	    ClpSimplex::isFree) {
...
// that would not have been done without positive edge
	if (modelPE_->doStatistics()) {
		if (bestDjComp < bestDj) modelPE_->addPriorityPivot();
	}
...
void 
ClpPEPrimalColumnSteepest::updateWeights(CoinIndexedVector * input)
{
  //if (modelPE_->isLastPivotCompatible()) {
